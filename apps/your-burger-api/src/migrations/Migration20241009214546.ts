...
export class Migration20241009214546 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`create table \`burger_brands\` (\`id\` integer not null primary key autoincrement, \`name\` text not null, \`slug\` text not null);`);
    ...
    this.addSql(`create table \`ingredient_categories\` (\`id\` integer not null primary key autoincrement, \`name\` text not null, \`i18n_key\` text not null);`);

    this.addSql(`create table \`ingredients\` (\`id\` integer not null primary key autoincrement, \`name\` text not null, \`category_id\` integer not null, \`i18n_key\` text not null, constraint \`ingredients_category_id_foreign\` foreign key(\`category_id\`) references \`ingredient_categories\`(\`id\`) on update cascade);`);
    this.addSql(`create index \`ingredients_category_id_index\` on \`ingredients\` (\`category_id\`);`);
